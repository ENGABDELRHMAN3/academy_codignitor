...
class Crud_model extends CI_Model
{
    ...
    public function get_student_lesson_watch_count($lessonId, $courseId, $userID = "") {
        if($userID ==""){
        ...
        $this->db->where('cours_id', $courseId);
       
        // print_r( $this->db->get('student_watches')->num_rows() .$lessonId."soursid:".$courseId ."userid ".$userId);
        return $this->db->get('student_watches')->num_rows();
    ...
    public function add_lesson()
    {
        ...
        $data['view_count'] = html_escape($this->input->post('count'));
        ...
        }elseif ($lesson_type == 'review') {
            $data['attachment'] = htmlspecialchars_(remove_js($this->input->post('text_question', false)[0]));
    ...
    public function GetReviewQuestions($reviewID= ""){
        if($reviewID != ""){
            $questions = $this->db->where('review_id',  $reviewID)->get('review_questions')->result_array();
