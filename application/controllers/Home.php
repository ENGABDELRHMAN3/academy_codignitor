...
class Home extends CI_Controller
{
    ...
    public function lesson($slug = "", $course_id = "", $lesson_id = "")
    {
        ...
        // print_r($this->crud_model->get_lessons('lesson', $lesson_id)->row_array());die;
                ...
                if ($lesson_id == "") {
                    ...
                    #check student view count
                     ...
                     if($studentViewCount <= $lesson->view_count-1|| $lesson_id== $firstLesson->id || $lesson->lesson_type == 'quiz' || $lesson->lesson_type == 'reviwe'){
                        ...
                        $default_section = $sections->row_array();
                        $page_data['section_id'] = $default_section['id'];
                        $lessons = $this->crud_model->get_lessons();
                        if ($lessons->num_rows() > 0 ) {
                            $default_lesson = $lessons->row_array();
                            $lesson_id = $default_lesson['id'];
        ...
        $viewData['lesson_id']= $lesson_id;
        $viewData['student_id']= $user_id;
        $viewData['cours_id']=$course_id;
